...
@section('sidebar-nav')
    @php $route = request()->routeIs('provider.dashboard') ? true : false; @endphp

    <a href="{{ route('provider.dashboard') }}" @class([
        'flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-colors',
        'bg-emerald-50 text-emerald-700' => request()->routeIs('provider.dashboard'),
        'text-gray-600 hover:bg-gray-50 hover:text-gray-800' => !request()->routeIs('provider.dashboard'),
    ])>
        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
        Dashboard
    </a>

    <a href="{{ route('provider.profile.edit') }}" @class([
        'flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-colors',
        'bg-emerald-50 text-emerald-700' => request()->routeIs('provider.profile.edit'),
        'text-gray-600 hover:bg-gray-50 hover:text-gray-800' => !request()->routeIs('provider.profile.edit'),
    ])>
    ...
    <a href="{{ route('provider.appointments.index') }}" @class([
        'flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-colors',
        'bg-emerald-50 text-emerald-700' => request()->routeIs('provider.appointments.*'),
        'text-gray-600 hover:bg-gray-50 hover:text-gray-800' => !request()->routeIs('provider.appointments.*'),
    ])>
        <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
        Appointments
        @if(isset($pendingAppointmentCount) && $pendingAppointmentCount > 0)
            <span class="ml-auto bg-rose-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full min-w-[20px] text-center">{{ $pendingAppointmentCount }}</span>
        @endif
    </a>

    <a href="{{ route('provider.messages.index') }}" @class([
        'flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-colors',
        'bg-emerald-50 text-emerald-700' => request()->routeIs('provider.messages.*'),
        'text-gray-600 hover:bg-gray-50 hover:text-gray-800' => !request()->routeIs('provider.messages.*'),
    ])>
