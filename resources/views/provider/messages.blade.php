...
<div class="mb-6">
    ...
    <p class="text-sm text-gray-500 mt-1">Messages with families, linked to appointment requests.</p>
</div>
...
@if($threads->isEmpty())
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
        ...
        <p class="text-gray-500 text-sm">Messages appear here once a family starts a conversation through an appointment request.</p>
    </div>
@else
    <div class="space-y-3">
        @foreach($threads as $thread)
            @php $lastMsg = $thread->messages->first(); @endphp
            <a href="{{ route('provider.messages.show', $thread) }}"
               class="block bg-white rounded-2xl border {{ $thread->unread_count > 0 ? 'border-emerald-300 shadow-md' : 'border-gray-100 shadow-sm' }} p-5 hover:shadow-md transition-shadow">
                <div class="flex items-start justify-between gap-3">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-1">
                            <p class="font-bold text-gray-800">{{ $thread->family->name }}</p>
                            @if($thread->unread_count > 0)
                                <span class="bg-emerald-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ $thread->unread_count }} new</span>
                            @endif
                        </div>
                        @if($lastMsg)
