    ...
    <div class="relative overflow-hidden">
        ...
        @auth
            @if(auth()->user()->isFamily())
                ...
                <form method="POST" action="{{ route('family.saved.toggle', $provider) }}"
                      class="absolute top-3 left-3" onclick="event.stopPropagation()">
                    @csrf
                    <button type="submit"
                            title="{{ $isSaved ? 'Remove from saved' : 'Save provider' }}"
                            class="w-8 h-8 rounded-full bg-white shadow-md flex items-center justify-center hover:scale-110 transition-transform">
                        @if($isSaved)
